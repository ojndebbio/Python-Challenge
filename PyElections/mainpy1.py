...
csvpath = os.path.join('Electionresults.csv')
...
with open(csvpath, newline='', encoding="utf-8") as csvfile:
   ...
   # Calculate and tally votes          
    ...
    f = open("PyElections_results.txt", "w")
    f.write(
    ...
    for x, y in sorted_cand_data.items():
        ...
        f = open("PyElections_results.txt", "a")
        f.write(f"{x}:  {z}%  ({y}) \n")
        f.close()
...
# Print out result
    f = open("PyElections_results.txt", "r")
    print(f.read())
    f.close()
